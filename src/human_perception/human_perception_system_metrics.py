...
config_direct=rospy.get_param("/hri_perception/config_direct","/home/Thorvald/rasberry_ws/src/mesapro/config/") #you have to change /hri_camera_detector/ if the node is not named like this
...
#Safaty parameters
camera_info=rospy.get_param("/hri_perception/camera_info",True) # flag to know if camera detector was activated or not, if not, then safety timer is not used
FP=0 #false positive
TP=0 #true positive
FN=0 #false negative
CGT=0 # Complete Ground Truth 
TN=0 # true negative, always 0
DR = TP/(TP+ FN) #detection rate
FAR = FP/(TP+FP) #false alarm rate
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():	
        ...
        if new_data[0]==1 or new_data[1]==1:
            ...
            if human.n_human>1:
                FP=FP+1
            elif (human.n_human==1 and (human.motion_track[0,0]+human.centroid_track[0,0]+human.centroid_track[0,1]+human.posture_track[0,0]+human.position_track[0,0]+human.position_track[0,1]+human.distance_track[0,0])!=0):
                TP=TP+1
            else:
                FN=FN+1
            CGT=CGT+1
