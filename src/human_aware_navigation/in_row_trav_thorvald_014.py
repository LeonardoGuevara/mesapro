...
#        laser_angles.append(msg.angle_max)
    ...
    def _get_references(self, publish_health=False):
        ...
        if not self.offset_row_detector:
            self.offset = None
        elif self.offset_row_detector and self.offset is None:
...
#        print dist, y_err, ang_diff
    ...
    def _get_vector_to_pose(self):
        ...
        elif self.row_poles is not None:
    ...
    def executeCallback(self, goal):
        ...
        if on_row:
            ...
            if self.offset_row_detector and self.offset is None:
                self.offset = self.get_row_offset()
                self.apply_offset_to_nodes()
...
if __name__ == '__main__':
    ...
    if not pole_positions_path.endswith('.yaml'):
        ...
        pole_pos_per_row = []
        for tunnel in pp:
            for row in tunnel[tunnel.keys()[0]]:
                coordinates = row[row.keys()[0]]["coordinates"]
                orientation = row[row.keys()[0]]["orientation"]
